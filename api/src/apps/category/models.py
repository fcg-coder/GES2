...
class Category(models.Model):  # Определяем модель Category, которая будет представлять категорию в базе данных
    ...
    parentCategory = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, related_name='child_pages')  # Поле для ссылки на родительскую категорию (если есть)
